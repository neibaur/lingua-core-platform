...
- Next action: Second friction triage complete (warrant-review-2026-06-09.md) — FIXTURE
  basis, no core-capability warrant; search-capability-shaped items (romanized-form lookup,
  as-you-type) HELD with clusters 5/8. Six app-tier presentation candidates identified, none
  authorized: direction-aware placeholder, direction-mismatch hint, user-facing error
  messages, URL-state persistence. Single next step — operator authorizes one as the next
  app-tier slice, or holds. Core gates unchanged: no core slice authorized; Phase 16/17
...
- Human-curated override / exception-table provenance — human-curation lineage and a
  ...
  before any reliance on or storage of generated tone. Three tone-related product decisions are now made on a provisional basis (operator): generator selection (tltk 1.10 G2P), canonical evaluation/storage notation (IPA with tone digits), and accuracy-validation methodology (human-annotated gold set); each is provisional and any reliance on, or storage representation of, generated tone falls under the validation gate stated immediately above.
