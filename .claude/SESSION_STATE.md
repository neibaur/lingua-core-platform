...
- Next action: Product decisions recorded (operator): tone IS a product requirement for UseThai; runtime tone
  inference is OUT; generated-and-stored offline/precomputed derived tone is the intended direction, subject to governance. Product/licensing posture recorded in DATA_SOURCES.md (Product Licensing Posture): commercial-first, ShareAlike-cautious — ShareAlike sources (e.g. Volubilis) remain candidates but ingestion stays gated until the specific ShareAlike obligations and product consequences are explicitly reviewed and accepted; do not proceed with Volubilis ingestion while it requires assuming ShareAlike tolerance. The derived-linguistic-artifact-provenance grounding thread is COMPLETE: an ARCHITECTURE.md "Derived Linguistic Artifact Provenance" amendment (grounding-of-record) plus companion ADR-0017 ground it as a concept distinct from source provenance (grounded case: machine-generated/precomputed pronunciation and tone; lineage = generator identity + generator version + input headword lineage; precompute-only, never runtime; not overloading DictionarySourceProvenance or the internal generatedFrom marker; human-curated override provenance deferred; no field introduced, concrete shape deferred to a later §9 assessment). Next concrete step: tone-validation pilot protocol now recorded as a planning artifact (docs/validation/tone-validation-pilot-protocol.md); pilot execution remains unauthorized — continue gathering REAL lookup friction via docs/usethai/ux-friction-log.md; Tone-generation product decisions (provisional baseline, operator — fork ruling: baseline-then-validate): generator = tltk 1.10 G2P, basis governance/lineage NOT accuracy (verified deterministic-at-pin; commercial-safe/posture-neutral BSD-3-Clause; smaller version surface; bundles own rule syllabification; simpler initial ADR-0017 lineage shape); ML options thaig2p/thaig2p_v2 (CC-BY-4.0) remain DEFERRED comparator candidates, revisitable only if validation shows the rule-based path insufficient or a later runnable environment verifies their determinism-at-pin and model-version pinning. Canonical evaluation/storage notation (candidate for now, NOT a final UI-display decision): IPA with tone digits, chosen for a cleaner cross-generator comparison path rather than binding to tltk's native scheme; a learner-friendly display layer may be considered later if warranted. Accuracy-validation methodology (approach decided, NOT yet authorized for execution): human-annotated gold set (chosen over a licensed reference dictionary to avoid a further licensing/governance dependency); sample over-represents the spike's known §4 failure categories — loanwords, proper nouns, ambiguous spellings, compounds, implicit vowels, and leading-consonant/false-cluster cases; scored by per-syllable tone accuracy (diagnostic metric) AND whole-word exact match (the stricter product-quality gate). These three product decisions (generator selection, canonical notation, accuracy-validation methodology) are now MADE on a provisional basis; reliance on generated tone remains gated on validation results (whole-word exact-match quality gate). Boundary confirmations: these rulings do NOT authorize ingestion, adapters, source synchronization, dependency changes, generated-data commits, a precompute pipeline, the concrete derived-provenance representation, the human-curated override/exception-table provenance thread, or executing the validation methodology (building/annotating/committing a gold set, adding any tooling or dependency — only the methodology is decided); choosing a generator does not clear Volubilis ingestion (Volubilis remains gated on the ShareAlike/product-posture decision). The §9 derived-provenance-shape assessment REMAINS UNAUTHORIZED and is now additionally gated on validation results (whole-word exact-match quality gate) before any reliance on or storage of generated tone. Prior triage context: First FIXTURE triage cycle COMPLETE. Cluster 1 (lookup
...
- Human-curated override / exception-table provenance — human-curation lineage and a
  ...
  deferred to a future §9 assessment, gated on validation results (whole-word exact-match quality gate)
  before any reliance on or storage of generated tone.
