...
Context
SESSION_STATE.md records the integration path as terminating at the Phase 13 search projections — ReadingPrimitiveSearchProjection, WritingPrimitiveSearchProjection, and SpellingEntrySearchProjection — with Phase 13 marked COMPLETE (all three slices merged) and Phase 14 recorded as the current phase, IN PROGRESS, with a repository-wide architectural audit complete and Phase 14 authorized to proceed (SESSION_STATE.md — Current Phase and Status; Active Scope and Derivation Surface). The roadmap concurs: Phase 13 COMPLETE, Phase 14 IN PROGRESS (ROADMAP.md — Phase Status).
...
This ADR is accepted and Phase 14 is authorized to proceed per SESSION_STATE.md — Current Phase and Status. Acceptance of this ADR does not by itself authorize implementation of any Phase 14 slice. Implementation authorization for each slice remains governed by SESSION_STATE.md and ROADMAP.md and requires a repository-first pre-implementation assessment per HANDOFF_TEMPLATE.md §9 before any code is written.
...
ROADMAP.md Phase 14 summary names four delivery categories as the exhaustive chartered scope of this phase: "Public application routes, API contracts, static and SEO-first rendering, and browser-native fallbacks." Each category is a structural classification (ARTIFACT CLASSIFICATION LAW: no evaluationTimestamp, no generatedFrom). A Phase 14 delivery contract represents exactly one of these categories. A foundational delivery primitive (defined in the Foundational Delivery Primitive subsection below) is authorized as a structural precursor to these categories and does not constitute a fifth chartered delivery category. No type outside the four chartered delivery categories and the foundational delivery primitive is authorized at this phase.
...
A foundational delivery primitive is an authorized structural type that establishes the typed connection between a Phase 13 search projection output and Phase 14 delivery identity. It is a necessary structural precursor to the four chartered delivery categories: the chartered category types compose from a foundational delivery primitive when introducing their category-specific content fields. The foundational delivery primitive is not itself a chartered delivery category.

Architectural role. The foundational delivery primitive derives exclusively from Phase 13 projection outputs and carries a canonical delivery identity and a typed reference to the source Phase 13 projection. It does not carry a content-bearing delivery field; that belongs to the chartered category types that compose from it. This two-layer sequence is grounded in the SINGLE-LAYER GOVERNANCE DERIVATION LAW: the foundational primitive's derivation from Phase 13 projections is one lawful single-layer step, and the chartered category types' derivation from the foundational primitive is a subsequent lawful single-layer step (HANDOFF_TEMPLATE.md §3 — SINGLE-LAYER GOVERNANCE DERIVATION LAW).

Parallel-type pattern. Phase 13 established three parallel projection types with no shared base or union in the repository (SESSION_STATE.md — Completed Slices; confirmed from repository source files). The existing Phase 13 parallel-type pattern may warrant corresponding parallel foundational delivery primitives at Phase 14. The determination of whether parallel types are warranted and how many remains subject to repository-first pre-implementation assessment under the DOCUMENTARY DERIVATION LAW and TYPED REFERENCE LAW (HANDOFF_TEMPLATE.md §3).

Structural classification. The foundational delivery primitive is a structural artifact per the ARTIFACT CLASSIFICATION LAW (HANDOFF_TEMPLATE.md §3). It carries no evaluationTimestamp and no generatedFrom field. Presence of either field on a foundational delivery primitive is a law violation.

Active laws. All active doctrine laws apply to every foundational delivery primitive and its builder, including without limitation: IMMUTABILITY LAW — deepFreezeStructure is required on all externally exposed artifacts (HANDOFF_TEMPLATE.md §3); REPLAY-SAFE GOVERNANCE LAW — caller-supplied identity only; no Date.now(), Math.random(), UUID generation, hash-derived identifiers, or crypto randomness at any step (HANDOFF_TEMPLATE.md §3); SINGLE-LAYER GOVERNANCE DERIVATION LAW — derives only from Phase 13 projection outputs; no lower-layer reach-through (HANDOFF_TEMPLATE.md §3); INVARIANT GUARD FORM LAW — all invariant guards execute directly and inline in the builder body as direct === equality comparisons or inline literal switch statements only; no delegated validation routines (HANDOFF_TEMPLATE.md §3); ARTIFACT CLASSIFICATION LAW — no evaluationTimestamp, no generatedFrom (HANDOFF_TEMPLATE.md §3). The TYPED REFERENCE LAW and the Canonical Delivery Identity definition in the subsection below govern how the foundational primitive's structural references are represented; the specific field names, field shapes, and structural membership are determined at pre-implementation assessment time under the DOCUMENTARY DERIVATION LAW (HANDOFF_TEMPLATE.md §3).

Names and shapes deferred. Exact type names, field names, builder signatures, and the specific naming relationship between each foundational delivery primitive and its corresponding Phase 13 projection source are determined at pre-implementation assessment time under the DOCUMENTARY DERIVATION LAW (HANDOFF_TEMPLATE.md §3). No type names, field names, or builder signatures are proposed or implied by this subsection.

Schema version convention. Schema version literals for foundational delivery primitive types follow the established repository convention: lingua-core-platform:<kebab-case-artifact-slug>@phase14 (SESSION_STATE.md — Schema Version Literals; HANDOFF_TEMPLATE.md §6 — Convention).
